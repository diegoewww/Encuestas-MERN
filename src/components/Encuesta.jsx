...
const Encuesta = () => {
    ...
    const getEncuesta = async () =>{
        const encuesta = await axios.get(`http://localhost:8800/api/v1/encuestas/${encuestaId}`)
        .then((res)=>{
    ...
    const handleSubmit = async (e) =>{
        e.preventDefault()
        if(voto){
            const info = await axios.post('http://localhost:8800/api/v1/encuestas/votar',voto)
            .then((res)=>{
                setToast({
                    snackbaropen:true,
                    msg:'Gracias por su participación, voto registrado!',
                    not:'success'
                });
                setTimeout (()=>{
            ...
            <form>
              {opciones.map((item) => (
                ...
                key={item.id} >
                  ...
                  <label role="button" htmlFor={item.id} className="flex align-items-center">
               <input
                      type="radio"
                      id={item.id}
                      name="opcion"
